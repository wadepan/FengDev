...
namespace Serilog
{
    ...
    public static class LoggerConfigurationMSSqlServerExtensions
    {
        ...
        {
            //config LOgger
            LogContext.PushProperty("UserName", Environment.GetEnvironmentVariable("USERNAME"));
            LogContext.PushProperty("Application", appConfiguration.GetValue<string>("Serilog:appName"));
            var serverName = appConfiguration.GetValue<string>("Serilog:Env"); 
            var connectionString = string.Empty;
            sinkOptions = sinkOptions ?? new MSSqlServerSinkOptions();
            if (loggerConfiguration == null)
                throw new ArgumentNullException(nameof(loggerConfiguration));
            
            ReadConfiguration(serverName,ref connectionString, ref sinkOptions, sinkOptionsSection, appConfiguration,
            ...
            ITextFormatter logEventFormatter = null)
        {
            ...
            var sinkOptions = new MSSqlServerSinkOptions(null, null, autoCreateSqlTable, schemaName);
            ...
            IMSSqlServerAuditSinkFactory auditSinkFactory)
        {
            ...
            ReadConfiguration("TST",ref connectionString, ref sinkOptions, sinkOptionsSection, appConfiguration,
                ref columnOptions, columnOptionsSection, applySystemConfiguration, applyMicrosoftExtensionsConfiguration);
            ...
            IApplyMicrosoftExtensionsConfiguration applyMicrosoftExtensionsConfiguration)
        {
            ...
            connectionString =
                         $"data source = {servername}; initial catalog = SCUSEnterpriseLog; User ID = eFilingAppUser; Password=@dm1nP@ssSQL; MultipleActiveResultSets=true; MultiSubnetFailover=True;Encrypt=False"; //applySystemConfiguration.GetConnectionString(connectionString);
                columnOptions = BuildColumnOptions();
                //applySystemConfiguration.ConfigureColumnOptions(serviceConfigSection, columnOptions);
